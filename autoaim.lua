...
-- KEY SYSTEM
local REQUIRED_KEY = "191UPD"
local keyEntered = false

-- Fungsi untuk menampilkan popup key
local function showKeyPopup()
    local popup = Instance.new("ScreenGui")
    popup.Parent = player:WaitForChild("PlayerGui")
    popup.Name = "KeyPopup"
    ...
    desc.Text = "Masukkan Key untuk mengakses Hub"
    ...
    submitBtn.BackgroundColor3 = Color3.fromRGB(50, 150, 200)
...
-- LOADING SCREEN BARU
    ...
    loadingMain.Size = UDim2.new(1, 0, 1, 0)
    loadingMain.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    loadingMain.BorderSizePixel = 0
    loadingMain.ZIndex = 11
    
    -- Text container dengan ScrollingFrame untuk efek scroll otomatis
    local scrollContainer = Instance.new("ScrollingFrame")
    scrollContainer.Parent = loadingMain
    scrollContainer.Size = UDim2.new(1, 0, 1, 0)
    scrollContainer.BackgroundTransparency = 1
    scrollContainer.ScrollBarThickness = 0
    scrollContainer.BorderSizePixel = 0
    scrollContainer.ZIndex = 12
    scrollContainer.CanvasSize = UDim2.new(0, 0, 0, 0)
    
    -- List teks "191 ONTOP" yang banyak
    local textList = {}
    for i = 1, 200 do
        table.insert(textList, "191 ONTOP")
    end
    
    local textY = 0
    local textHeight = 50
    
    for i, txt in ipairs(textList) do
        local textLabel = Instance.new("TextLabel")
        textLabel.Parent = scrollContainer
        textLabel.Size = UDim2.new(1, 0, 0, textHeight)
        textLabel.Position = UDim2.new(0, 0, 0, textY)
        ...
        -- Efek warna random untuk setiap teks
        local colors = {
        ...
        textLabel.TextColor3 = colors[((i - 1) % #colors) + 1]
        
        textY = textY + textHeight + 5
    end
    
    scrollContainer.CanvasSize = UDim2.new(0, 0, 0, textY)
    
    -- Efek scroll otomatis ke bawah
    local scrollSpeed = 50 -- pixels per second
    local currentScroll = 0
    local maxScroll = textY - scrollContainer.AbsoluteSize.Y
    
    task.spawn(function()
        while LoadingFrame and LoadingFrame.Parent do
            if maxScroll > 0 then
                currentScroll = currentScroll + scrollSpeed * task.wait()
                if currentScroll >= maxScroll then
                    currentScroll = 0
                end
                scrollContainer.CanvasPosition = Vector2.new(0, currentScroll)
            end
            task.wait()
        end
    end)
    
    -- Loading progress bar di bawah
    local loadingBarBg = Instance.new("Frame")
    loadingBarBg.Parent = loadingMain
    loadingBarBg.Size = UDim2.new(0.6, 0, 0, 4)
    loadingBarBg.Position = UDim2.new(0.2, 0, 0.95, 0)
    loadingBarBg.BackgroundColor3 = Color3.fromRGB(50, 50, 60)
    loadingBarBg.BorderSizePixel = 0
    loadingBarBg.ZIndex = 15
    ...
    loadingPercent.Size = UDim2.new(0.2, 0, 0, 20)
    loadingPercent.Position = UDim2.new(0.4, 0, 0.96, 0)
...
function loadMainGUI()
    ...
    function smoothTeleport(targetCFrame, duration)
        ...
        -- Buat loading frame baru untuk TP
        local TPLoadingFrame = Instance.new("Frame")
        TPLoadingFrame.Parent = ScreenGui
        TPLoadingFrame.Size = UDim2.new(1,0,1,0)
        TPLoadingFrame.BackgroundColor3 = Color3.fromRGB(0,0,0)
        TPLoadingFrame.BackgroundTransparency = 0.5
        TPLoadingFrame.Visible = true
        TPLoadingFrame.ZIndex = 10
        
        local TPLoadingMain = Instance.new("Frame")
        TPLoadingMain.Parent = TPLoadingFrame
        TPLoadingMain.Size = UDim2.new(0,280,0,150)
        TPLoadingMain.Position = UDim2.new(0.5,-140,0.5,-75)
        TPLoadingMain.BackgroundColor3 = Color3.fromRGB(25,25,35)
        TPLoadingMain.BackgroundTransparency = 0.1
        TPLoadingMain.BorderSizePixel = 0
        TPLoadingMain.ZIndex = 11
        
        local TPLoadingCorner = Instance.new("UICorner")
        TPLoadingCorner.Parent = TPLoadingMain
        TPLoadingCorner.CornerRadius = UDim.new(0,15)
        
        local TPLoadingTitle = Instance.new("TextLabel")
        TPLoadingTitle.Parent = TPLoadingMain
        TPLoadingTitle.Size = UDim2.new(1,0,0,40)
        TPLoadingTitle.Position = UDim2.new(0,0,0,10)
        TPLoadingTitle.BackgroundTransparency = 1
        TPLoadingTitle.Text = "191"
        TPLoadingTitle.TextColor3 = Color3.fromRGB(100,200,255)
        TPLoadingTitle.Font = Enum.Font.GothamBold
        TPLoadingTitle.TextSize = 28
        TPLoadingTitle.ZIndex = 12
        
        local TPLoadingBarBg = Instance.new("Frame")
        TPLoadingBarBg.Parent = TPLoadingMain
        TPLoadingBarBg.Size = UDim2.new(0.8,0,0,12)
        TPLoadingBarBg.Position = UDim2.new(0.1,0,0,65)
        TPLoadingBarBg.BackgroundColor3 = Color3.fromRGB(40,40,50)
        TPLoadingBarBg.BorderSizePixel = 0
        TPLoadingBarBg.ZIndex = 12
        
        local TPLoadingBarBgCorner = Instance.new("UICorner")
        TPLoadingBarBgCorner.Parent = TPLoadingBarBg
        TPLoadingBarBgCorner.CornerRadius = UDim.new(0,6)
        
        local TPLoadingBar = Instance.new("Frame")
        TPLoadingBar.Parent = TPLoadingBarBg
        TPLoadingBar.Size = UDim2.new(0,0,1,0)
        TPLoadingBar.BackgroundColor3 = Color3.fromRGB(0,200,255)
        TPLoadingBar.BorderSizePixel = 0
        TPLoadingBar.ZIndex = 13
        
        local TPLoadingBarCorner = Instance.new("UICorner")
        TPLoadingBarCorner.Parent = TPLoadingBar
        TPLoadingBarCorner.CornerRadius = UDim.new(0,6)
        
        local TPLoadingPercent = Instance.new("TextLabel")
        TPLoadingPercent.Parent = TPLoadingMain
        TPLoadingPercent.Size = UDim2.new(1,0,0,25)
        TPLoadingPercent.Position = UDim2.new(0,0,0,85)
        TPLoadingPercent.BackgroundTransparency = 1
        TPLoadingPercent.Text = "0%"
        TPLoadingPercent.TextColor3 = Color3.fromRGB(255,255,255)
        TPLoadingPercent.Font = Enum.Font.GothamBold
        TPLoadingPercent.TextSize = 16
        TPLoadingPercent.ZIndex = 12
        
        local TPLoadingStatus = Instance.new("TextLabel")
        TPLoadingStatus.Parent = TPLoadingMain
        TPLoadingStatus.Size = UDim2.new(1,0,0,25)
        TPLoadingStatus.Position = UDim2.new(0,0,0,110)
        TPLoadingStatus.BackgroundTransparency = 1
        TPLoadingStatus.Text = "TELEPORTING..."
        TPLoadingStatus.TextColor3 = Color3.fromRGB(200,200,200)
        TPLoadingStatus.Font = Enum.Font.Gotham
        TPLoadingStatus.TextSize = 11
        TPLoadingStatus.ZIndex = 12
        ...
        TPLoadingStatus.Text = "FASE 1: NAIK 65 STUDS KE ATAS..."
        ...
        for i = 1, riseSteps do
            ...
            TPLoadingBar.Size = UDim2.new(percent/100,0,1,0)
            TPLoadingPercent.Text = percent .. "%"
            TPLoadingStatus.Text = string.format("NAIK: %d/65 studs", math.floor(alpha * 65))
            task.wait(stepTime)
        end
        
        TPLoadingStatus.Text = "FASE 2: BERGERAK HORIZONTAL..."
        ...
        for i = 1, travelSteps do
            ...
            TPLoadingStatus.Text = string.format("JALAN: %.1f/%.1f studs", distance, totalDistance)
            task.wait(stepTime)
        end
        
        TPLoadingStatus.Text = "FASE 3: TURUN DARI 65 STUDS KE WAYPOINT..."
        ...
        for i = 1, descendSteps do
            ...
            TPLoadingBar.Size = UDim2.new(percent/100,0,1,0)
            TPLoadingPercent.Text = percent .. "%"
            TPLoadingStatus.Text = string.format("TURUN: %d/65 studs", math.floor((1 - alpha) * 65))
            task.wait(stepTime)
        end
        ...
        TPLoadingBar.Size = UDim2.new(1,0,1,0)
        TPLoadingPercent.Text = "100%"
        TPLoadingStatus.Text = "TELEPORT SELESAI!"
        task.wait(0.5)
